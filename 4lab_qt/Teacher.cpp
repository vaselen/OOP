...
void vasuikova_Teacher::updateFromEditDialog(const EditDialog* dialog) {
    if (!dialog) return;
    setName(dialog->getName().toStdWString());
    setAge(dialog->getAge());
    setDanceStyle(dialog->getStyle().toStdWString());
}

void vasuikova_Teacher::setupEditDialog(EditDialog* dialog) const {
    if (!dialog) return;
    dialog->setName(QString::fromStdWString(getName()));
    dialog->setAge(getAge());
    dialog->setStyle(QString::fromStdWString(getDanceStyle()));
    dialog->setDirectorMode(false);
}

void vasuikova_Teacher::fillFields(TeacherListDialog* dialog) const {
    if (!dialog) return;

    dialog->setName(QString::fromStdWString(getName()));
    dialog->setAge(getAge());
    dialog->setStyle(QString::fromStdWString(getDanceStyle()));
    dialog->showDirectorFields(false);
}
